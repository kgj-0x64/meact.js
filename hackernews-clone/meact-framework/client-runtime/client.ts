...
export function _initRender(
  ...
  if (!targetNodeInBrowserDom) {
    console.error(
      `Please check if you have accidently removed '<div id="root"></div>' element from the "body" of "index.html" document`
    );
    return;
  }
  ...
  const renderTreeRootNode = MyApp({
    PageComponentFn,
    pageProps,
  });
